...
def assert_valid_statement(uri: str):
    ...
    if not match:
        raise AssertionError(f"URI >{uri}< does neither match hostname >creativecommons.org< nor hostname >rightsstatements.org<.")
    ...
    match hostname:
        case "creativecommons.org":
            ...
            if not match:
                raise AssertionError(f"URI >{uri}< does not match any of the creative commons licenses.")

        case "rightsstatements.org":
            match = rights_statements_pattern.match(path)
            if not match:
                raise AssertionError(f"URI >{uri}< does not match any of the rights statements licenses.")
